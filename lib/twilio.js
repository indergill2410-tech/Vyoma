...
export async function send({ to, body, channel = "sms" }) {
  ...
  const From = whatsapp ? `whatsapp:${fromRaw.replace(/^whatsapp:/, "")}` : fromRaw;
  const To = whatsapp ? `whatsapp:${to.replace(/^whatsapp:/, "")}` : to;
...
export const messages = {
  orderPlaced: (orderNumber) =>
    `✦ Vyoma — your order ${orderNumber} is on its way to being made in Tiruppur. We'll text you when it ships. Room to grow.`,
